...
from .hep import nll, fit_process

class HEPMixture:
    """
    EM algorithm for mixture of Hierarchical Excitatory Point Processes.
    Each sequence of events conditional on latent cluster assignment z_i=k follows a 
    Hierarchical Excitatory Point Process with cluster-specific parameters theta_k.
    Model: Event sequence i | z_i=k ~ HEP(lambda_k(t; theta_k))    
    ...
    def init_params(self, event_times, observation_intervals=None, init_method='kmeans', offset=1e-12, random_state=None):
        ...
        if hasattr(self, 'T') and self.T is not None:
            ...
            T = int(np.ceil(max([max(times) if len(times) > 0 else 0 for times in event_times]))) + 1e-4
            self.T = T
        ...
        # Initialize HEP process parameters for each cluster
        ...
        # Update HEP process parameters for each cluster
    ...
    def calculate_T(self, event_times, T=None):
        ...
        if T is None:
            ...
            T = int(np.ceil(max([max(event_times[node]) if len(event_times[node]) > 0 else 0 for node in event_times]))) + 1e-4
        else:
    ...
    def fit(self, event_times, observation_intervals=None, T=None, init_method='kmeans', offset=1e-12, random_state=None):
        ...
        if T is not None:
            ...
            if hasattr(self, 'T') and self.T is not None:
                ...
                # Determine T from the maximum event time across all sequences
                T = int(np.ceil(max([max(times) if len(times) > 0 else 0 for times in event_times_list]))) + 1e-4
                self.T = T
        ## Check if self.observation_intervals is set
        if hasattr(self, 'obs_intervals') and observation_intervals is not None:
            raise ValueError("observation_intervals has already been set. To recalculate, please create a new instance of the class.")
    ...
    def ICL(self, event_times, observation_intervals=None, T=None, init_method='kmeans', offset=1e-12, random_state=None):
        """
        Fit the mixture model using the EM algorithm.
        ...
        # Calculate number of parameters based on configuration
        n = len(event_times_list)
        nu = self.K * self.theta.shape[1] + self.K - 1  # Total number of parameters (process parameters + mixture proportions)
        # Calculate ICL criterion
        icl = ll - nu * np.log(n) / 2 + (resp * np.log(resp + 1e-12)).sum()
        return icl ## , ll, nu * np.log(n) / 2, (resp * np.log(resp + 1e-12)).sum()